...
function buildSide(dateStart, dateEnd, city, country, timeZone) {
  ...
  return {
    ...
    country,
    timezone: timeZone,
    localDate: localDateKey(dateStart, timeZone),
    dateLabel: formatLongDate(dateStart, timeZone),
    timeRange: `${formatTime(dateStart, timeZone)} – ${formatTime(dateEnd, timeZone)}`,
    weekend,
  };
}
